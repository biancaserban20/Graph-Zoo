...
{-
    *** TODO ***
    ...
    (1*(2+3))
-}
printGraf :: (Show a) => AlgebraicGraph a -> String
printGraf Empty = ['_'] -- pentru mine
printGraf (Node a) = show a
printGraf (Overlay g1 g2) = ['('] ++ (printGraf g1) ++ ['+'] ++ (printGraf g2) ++ [')']
printGraf (Connect g1 g2) = ['('] ++ (printGraf g1) ++ ['*'] ++ (printGraf g2) ++ [')']
instance Show a => Show (AlgebraicGraph a) where
    show graph = printGraf graph 
...
splitNode node targets = extend (\n -> if n == node then (foldl Overlay Empty (map Node targets)) else (Node n)) 
