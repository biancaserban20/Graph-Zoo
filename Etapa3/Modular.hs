...
-}
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f xs = myFunc (take (length xs) [0..]) xs []
 where myFunc index lista acc = if index == []
                                then reverse acc
                                else  myFunc (tail index) lista (new : acc)
                                where 
                                 new = first ++ [newElem] ++ rest
                                  where 
                                   first = fst(splitAt (head index) lista)
                                   newElem = f (head (snd (splitAt (head index) lista)))
                                   rest = tail (snd (splitAt (head index) lista))
...
partitions xs  
        | length xs == 0 = [ [ [ ] ] ]
        | length xs == 1 = [ [ [ head xs ] ] ] 
        | otherwise = (map ( [[head xs]]++ ) (partitions (tail xs))) ++ ( foldl (\x y -> x ++ y) [] (map (mapSingle ((head xs) :)) (partitions (tail xs))))
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = if (S.map (\n -> (funct n graph)) set) == (S.fromList [True]) then True else False
    where funct n graph = if (S.difference (inNeighbors n graph) set) == x && (S.difference (outNeighbors n graph) set) == y
                           then True
                           else False
                            where x = S.difference (inNeighbors (head (S.toList set)) graph) set 
                                  y = S.difference (outNeighbors (head (S.toList set)) graph) set
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = if (S.map (\n -> (isModule n graph)) partition) == (S.fromList [True]) then True else False
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = minimumBy (compare `on` func) (filter (\n -> isModularPartition n graph) partitions)
 where func = (\x -> if (S.size x) <= 1 then 1000 else (S.size x))
{-
